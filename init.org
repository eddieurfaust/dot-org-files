...
** avy, a better =ace-jump=
   ...
   #+END_QUOTE
   - =avy-keys= erwartet offenbar eine Liste mit allen Buchstaben [a-z]. Der "unterliegende Code" wird offenbar mit dem <?> Operator zurück gegeben.
...
(use-package avy
  ...
  :config
  (setq avy-keys '(?n ?e ?r ?a ?s ?o ?t ?i ?d ?u ?y ?h ?c ?g ?l ?f ?v ?k ?w ?q ?x ?b ?p ?m ?z ?j))
  (setq avy-keys-alist '((avy-goto-word-or-subword-1 . (?n ?e ?r ?a ?s ?o ?t ?i))
			 (avy-goto-word-1 . (?n ?e ?r ?a ?s ?o ?t ?i))
			 (avy-goto-char-2 . (?n ?e ?r ?a ?s ?o ?t ?i))
			 (avy-goto-line . (?n ?e ?r ?a ?s ?o ?t ?i))))
